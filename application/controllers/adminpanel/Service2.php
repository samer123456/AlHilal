...
class Service2 extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['service_ar'] = $this->Database_Model->getContentWithId(1, 'buyfromturkey1');
		$data['service_tr'] = $this->Database_Model->getContentWithId(2, 'buyfromturkey1');
		$data['service_en'] = $this->Database_Model->getContentWithId(3, 'buyfromturkey1');
		...
		$this->load->view('admin/service2', $data);
	}
	...
	public function updateService($id)
	{
		...
		$this->Database_Model->updateWithId('buyfromturkey1', $data, 'id', $id);
		redirect(base_url() . 'adminpanel/service2');
	}
	...
	function loadImage($id)
	{
		...
		$data['numberUnMsg'] = $this->db->query('SELECT id FROM message WHERE status=0')->num_rows();
		$data['unreadMessage'] = $this->Database_Model->getContentId(0, 'message', 'status');
		$this->load->view('admin/loadImageService2', $data);
	}
	...
	{
		$this->Database_Model->deleteWithId('buyfromturkey1', 'id', $id);
		redirect(base_url() . 'adminpanel/service2');
	}
}
